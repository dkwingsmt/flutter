...
import '../painting/mocks_for_image_cache.dart' hide TestImage;
...
void main() {
  ...
  testWidgets("ImageMouseCursor doesn't repaint after preparation for the first time, and is sync for the second time", (WidgetTester tester) async {
    final List<_CursorUpdateDetails> logCursors = <_CursorUpdateDetails>[];
    _setMockCursorHandlers(logCursors: logCursors);
    addTearDown(_clearMockCursorHandlers);
    ...
    final ImageProvider provider = _TestImageProvider(1, 2, image: _testSquareImage(5));
    ...
    await tester.pumpWidget(_Scaffold(
      topLeft: Container(
        ...
        width: 10,
        child: MouseRegion(
          cursor: ImageMouseCursor(image: provider, hotSpot: const Offset(1, 2)),
          child: CustomPaint(painter: _DelegatedPainter(onPaint: onPaintChild)),
        ),
      ),
      background: const MouseRegion(cursor: SystemMouseCursors.click),
    ));

    expect(paintLogs, <String>['paint']);
    expect(logCursors, const <_CursorUpdateDetails>[
      _CursorUpdateDetails(device: 1, cursor: SystemMouseCursors.click),
    ]);
    paintLogs.clear();
    logCursors.clear();
    ...
    expect(paintLogs, isEmpty);
    expect(logCursors, const <_CursorUpdateDetails>[
      _CursorUpdateDetails(device: 1, cursor: PreparedImageMouseCursor(imageId: 20)),
    ]);
    paintLogs.clear();
    logCursors.clear();
    ...
    await tester.pumpWidget(_Scaffold(
      topLeft: Container(
        key: const ValueKey<String>('another'),
        height: 150,
        width: 150,
        child: MouseRegion(
          cursor: ImageMouseCursor(image: provider, hotSpot: const Offset(1, 2)),
...
class _TestImageProvider extends TestImageProvider {
  const _TestImageProvider(int key, int imageValue, { ui.Image image })
...
ui.Image _testSquareImage(int size) {
  return _MockImage(
    width: size,
    height: size,
    rgbaColors: (int x, int y) => x << 4 | y,
  );
}

class _MockImage implements ui.Image {
  // Create an image with the specific size and colors of each pixel.
  _MockImage({
    int width,
    int height,
    int Function(int x, int y) rgbaColors,
  }) : _width = width,
       _height = height,
       _data = _buildData(width, height, rgbaColors);
  ...
  Future<ByteData> toByteData({ ui.ImageByteFormat format = ui.ImageByteFormat.rawRgba }) async {
    switch (format) {
      case ui.ImageByteFormat.rawRgba:
        return _data;
      default:
        throw UnsupportedError('Cannot encode test image in this format');
    }
  }
  ...
  static ByteData _buildData(int width, int height, int Function(int x, int y) pixelColors) {
    final ByteBuffer buffer = Uint8List(width * height * 4).buffer;
    final ByteData bdata = ByteData.view(buffer);

    int offset = 0;
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        bdata.setInt32(offset, pixelColors(x, y));
        offset += 4;
      }
    }
    return bdata;

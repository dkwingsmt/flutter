...
class PhysicalKeyboardKey extends KeyboardKey {
  ...
  /// A description representing the position of the key.
  ///
  /// This value is useful for providing readable strings for keys or keyboard
  /// shortcuts when the position of the key matters instead of its logical
  /// representation. Do not use this value to compare equality of keys; compare
  /// [usbHidUsage] instead.
  ///
  /// The returned string is always a readable string that describes the key,
  /// using the US keyboard layout if applicable, such as 'Key A', 'Numpad 1',
  /// 'Media Play'.
  ///
  /// This value is an empty string if there's no key label data for a key.
  String get keyLabel {
    return _keyLabels[usbHidUsage] ?? '';
  }

  /// The debug string to print for this keyboard key, which will be null in
  /// release mode.
  ///
  /// This value tries to return [keyLabel] if available, a readable string that
  /// describes the key, using the US keyboard layout if applicable, such as 'Key
  /// A', 'Numpad 1', 'Media Play'.
  ///
  /// If there's no key label data for a key, this returns a name that
  /// explains the ID (such as 'HID 0x700ff').
  String? get debugName {
    String? result;
    assert(() {
      result = _keyLabels[usbHidUsage] ??
        'HID 0x${usbHidUsage.toRadixString(16).padLeft(5, '0')}';
      return true;
    }());
    return result;
  }

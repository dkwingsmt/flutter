...
class RsePathBuilderImpellerDelegate
    : public RoundSuperellipseParam::PathBuilderDelegate {
 public:
  RsePathBuilderImpellerDelegate(PathBuilder& builder) : builder_(builder) {}

  void MoveTo(const Point& point) override { builder_.MoveTo(point); }

  void CubicCurveTo(const Point& p2,
                    const Point& p3,
                    const Point& p4) override {
    builder_.CubicCurveTo(p2, p3, p4);
  }

  void LineTo(const Point& point) override { builder_.LineTo(point); }
...
PathBuilder& PathBuilder::AddRoundSuperellipse(RoundSuperellipse rse) {
  ...
  RsePathBuilderImpellerDelegate delegate(*this);
  auto param =

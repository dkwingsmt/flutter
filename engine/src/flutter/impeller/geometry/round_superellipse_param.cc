...
class RoundSuperellipseBuilder {
 public:
  ...
  // `cubic_adder`.
  explicit RoundSuperellipseBuilder(
      RoundSuperellipseParam::PathBuilderDelegate& delegate)
      : delegate_(delegate) {}
  ...
  void AddQuadrant(const RoundSuperellipseParam::Quadrant& param,
    ...
    auto transform = Matrix::MakeTranslateScale(param.signed_scale * scale_sign,
                                                param.offset);
    if (param.top.se_n < 2 || param.right.se_n < 2) {
      delegate_.LineTo(transform * (param.top.offset +
                                    Point(param.top.se_a, param.top.se_a)));
      if (!reverse) {
        delegate_.LineTo(transform *
 ...
 private:
  ...
  void AddOctant(const RoundSuperellipseParam::Octant& param,
    ...
    if (!reverse) {
      delegate_.CubicCurveTo(transform * se_points[1], transform * se_points[2],
                             transform * se_points[3]);
      delegate_.CubicCurveTo(transform * circle_points[1],
                             transform * circle_points[2],
                             transform * circle_points[3]);
    } else {
      delegate_.CubicCurveTo(transform * circle_points[2],
                             transform * circle_points[1],
                             transform * circle_points[0]);
      delegate_.CubicCurveTo(transform * se_points[2], transform * se_points[1],
  ...
  RoundSuperellipseParam::PathBuilderDelegate& delegate_;
...
void RoundSuperellipseParam::AddPath(PathBuilderDelegate& delegate) const {
  RoundSuperellipseBuilder builder(delegate);

  Point start = top_right.offset +
                top_right.signed_scale *
                    (top_right.top.offset + Point(0, top_right.top.se_a));
  delegate.MoveTo(start);
